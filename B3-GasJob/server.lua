...
AddEventHandler('damn_lenhador:rewardWood', function()
	...
	if vRP.hasPermission({user_id,"gas.legal"}) then
		local new_weight = vRP.getInventoryWeight({user_id})+vRP.getItemWeight({"gascan"})
		if new_weight <= vRP.getInventoryMaxWeight({user_id}) then
			vRP.giveInventoryItem({user_id,"gascan",rndResult,false})
			TriggerClientEvent('chatMessage', source, '', {255,192,203}, "تلقيت "..rndResult.." اسطوانة غاز.", {255, 255, 255, 0.5,'',0, 100, 0, 0.50})
		else
			TriggerClientEvent('chatMessage', source, '', {255,192,203}, "الحقيبة مكتملة", {255, 255, 255, 0.5,'',100,0,0, 0.5})
		end
	else
      	TriggerClientEvent('chatMessage', source, '', {255,192,203}, "انت لست صانع انابيب غاز", {255, 255, 255, 0.5,'',100,0,0, 0.5}, "Hit", "RESPAWN_SOUNDSET")
    end
end)
...
AddEventHandler('damn_lenhador:processWood', function()
	local user_id = vRP.getUserId({source})
	if vRP.hasPermission({user_id,"gas.legal"}) then
		if vRP.tryGetInventoryItem({user_id,"gascan",1,true}) then
